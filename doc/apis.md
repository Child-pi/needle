...
- `extract()` caps the engine at one call per turn, so a record schema never yields a second record; `escalate` is also raised when the request negates the tool's verb, quotes a command reported by someone else, or a required enumerated value has no support in the request.
