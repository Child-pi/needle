...
| `agent.complete(text="", max_new_tokens=512, audio=None, audio_format="wav", sample_rate=0, channels=1)` | Complete a text, speech, or mixed turn. `audio` accepts WAV bytes or a path; PCM16 and float32 buffers use the accompanying format, rate, and channel fields. |
...
| `agent.run(query="", max_steps=8, max_new_tokens=512, audio=None, audio_format="wav", sample_rate=0, channels=1, strict=True)` | Full agentic loop: the model picks calls, Needle executes your Python functions and feeds results back, and the final response carries the executed tool results as `results`. |
...
- An escalated call is withheld by the engine: `function_calls` is empty and `suppressed_calls` carries it. The agent prefixes the local `date:` fact to `system` automatically (`auto_date=False` opts out) so relative dates resolve.
...
- Speech: `complete(text, audio=clip)` transcribes the clip in a clean context, then answers a text turn that carries the transcript wrapped in `<context>` followed by `text` (or "Fulfill the spoken request." when `text` is empty); the speech codes are not part of that turn and the transcript is returned as `transcript` with `transcript_confidence` (geometric mean token probability of the transcription decode).
- `extract()` declares the record as the only tool; a turn with a single declared tool is a record extraction, so an escalated record is returned with the `escalate` flag rather than withheld, and `strict` handles grounding.
- Arguments arrive after the engine's deterministic repair step, which grounds them in the request text (first/last name split, phone completion, verbatim quoted titles, place queries with their street address, weekday and explicit dates resolved against the `date:` fact, empty optionals and duplicate calls dropped). Enumerated parameters are never altered.
...
The `confidence` field is the minimum of two signals: a calibrated post-hoc head that scores the full prompt plus the call the model just produced, and the decoding probability of the call tokens. A call is accepted only when both agree, so the failure mode is escalation, not wrong execution. The contract: pick a threshold for your product, act at or above it, re-ask or route to a bigger model below it. Off-topic requests return the empty call `[]`. Every response also carries `escalate`, true when `confidence` is below the engine's default threshold of 0.1.
...
Each generation's engine is fetched once and cached under `~/.cache/cactus-needle/v2/` or `v3/`; Needle 3 also caches its base archive `needle3.cact` there, since the engine carries no weights (`needle download needle3` fetches it explicitly). Inference itself never touches the network, so an air gapped device only needs the matching files in place. Three ways to get them there:
