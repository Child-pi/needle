...
| `agent.complete(text="", max_new_tokens=512)` | One turn. You execute the call and feed the result back yourself via the next `complete(...)`. |
| `agent.embed(text="")` | Embed text or serialized tool schemas with the Needle 3 retrieval head. |
| `agent.run(query="", max_steps=8, max_new_tokens=512, strict=True)` | Full agentic loop: the model picks calls, Needle executes your Python functions and feeds results back, and the final response carries the executed tool results as `results`. |
...
- A call below the confidence threshold, or contradicted by the request, is withheld by the engine: `function_calls` is empty and `suppressed_calls` carries it. The agent prefixes the local `date:` fact to `system` automatically (`auto_date=False` opts out) so relative dates resolve.
...
- `extract()` declares the record as the only tool and takes the record from `function_calls` or, when the engine withheld it, from `suppressed_calls`; `strict` handles grounding.
...
The `confidence` field is the minimum of two signals: a calibrated post-hoc head that scores the full prompt plus the call the model just produced, and the decoding probability of the call tokens. A call is accepted only when both agree, so the failure mode is escalation, not wrong execution. The contract: pick a threshold for your product, act at or above it, re-ask or route to a bigger model below it. Off-topic requests return the empty call `[]`. A call below the engine's default threshold of 0.1 is withheld into `suppressed_calls`.
